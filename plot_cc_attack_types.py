...
def latest_run(df: pd.DataFrame, min_rounds: int) -> pd.DataFrame:
    ...
    # Se o Pandas confundir a primeira linha de dados com o cabecalho, conserte automaticamente:
    if len(df.columns) >= 8 and ('FPR' in df.columns or 'benign' in df.columns or '5' in df.columns):
        # Renomeia as colunas para o formato correto
        df.columns = ['run_id', 'round', 'cc', 'attack_type', 'total', 'removed', 'rate', 'metric'][:len(df.columns)]
    # Automatically find the column name by ignoring spaces and capitalization
    col_name = next((c for c in df.columns if c.strip().lower() == 'round'), None)
    
    # If the CSV is completely empty or missing the column, safely skip
    if col_name is None:
        print(f"⚠️ Aviso: Nenhuma coluna 'round' encontrada. Colunas disponiveis: {df.columns.tolist()}")
        return df
        
    rounds = df[col_name].astype(int)
